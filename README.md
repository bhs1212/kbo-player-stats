...
**핵심**: KBO 공식 사이트 크롤링 + 박스스코어 API 통합 + AI 챗봇(RAG) 시즌 685경기 + 265개 박스스코어 자동 적재

## 📌 프로젝트 소개

KBO 공식 사이트에서 선수 기록과 경기 일정을 자동으로 수집해 보여주고 회원가입한 사용자는 응원팀을 설정해 개인화된 일정 화면을 받을 수 있습니다. 박스스코어 API를 별도로 통합해 경기별 상세 데이터(이닝 점수, 타자/투수 출장 기록)도 제공하며 박스스코어 누적 데이터와 시즌 통계를 비교해 데이터가 맞는지 매일 확인합니다. Claude Haiku API 기반 챗봇으로 선수 기록을 자연어로 질문할 수 있고 동일 질문 캐싱과 IP별 요청 제한으로 호출 비용을 관리합니다.
...
| Rate Limit   | Bucket4j                                  |
| Frontend     | Thymeleaf, Bootstrap 5, FullCalendar v6   |
...
│   └── BoxScoreCrossValidationService.java  # 박스스코어 ↔ 시즌 통계 비교
├── scheduler/
...
- **game_event**: 주요 이벤트 (홈런, 도루, 실책, 폭투 등)
...
**현재 적재량**: 선수 460명, 경기 일정 685건, 박스스코어 265경기, 검증 로그 668건
...
- FullCalendar v6 캘린더 뷰 (모바일 자동 리스트뷰 전환)
- 게임 클릭 시 모달로 상세 표시
- 캘린더 ↔ 카드 그리드 토글 (localStorage로 선호 뷰 기억)
...
- 5개 정규화 테이블 (1:N + ON DELETE CASCADE), `INSERT ON DUPLICATE KEY UPDATE` + `deleteByGameId`로 여러 번 돌려도 같은 결과 보장
- Spring Cache(`@Cacheable`) + `@CacheEvict`로 GET 최적화 및 데이터 변경 시 자동 무효화
- 박스스코어 도루 표기 해석으로 도루 정확성 100% (KBO 공식 통계와 일치)
- Thymeleaf 페이지 + REST API 이중 노출 (동일 서비스 재사용)
- 시즌 전체 265경기 박스스코어 적재 + KBO 공식과 1:1 정합성 검증
- 박스스코어 합산과 시즌 통계를 비교해 매일 데이터 정확성 확인
- 매일 23:50 자동 수집 스케줄러

### AI 챗봇

- Anthropic Claude Haiku API 사용
- RAG 방식 - DB의 실제 선수 데이터를 컨텍스트로 전달해 AI가 데이터 없이 답 지어내는 거 방지
...
**결론**: 차이는 우리 시스템이 아니라 KBO 박스스코어 페이지와 시즌 통계 페이지의 집계 차이 비교 도구가 없었으면 못 발견했을 외부 데이터 소스의 한계

### 5. 박스스코어 도루 표기 해석

**문제**: 박민우(NC) 도루가 KBO 공식엔 19개인데 우리는 11개

**원인**: 박스스코어 도루 표기에 이름 뒤 숫자가 도루 횟수였는데 우리 파서는 그냥 무시

```
"박민우(3회)"      → 3회에 1번 도루
"박민우2(1회)"     → 1회에 2번 도루 (1루→2루→3루)
"박민우2(1 6회)"   → 1회 + 6회에 각각 1번
"이주형3(1 5 9회)" → 1, 5, 9회에 각각 1번
```

**해결**: 정규식을 `(이름)(횟수)(회차들)` 3그룹으로 다시 짜서 횟수만큼 이벤트 생성 박민우 19개로 KBO 공식과 일치
...
| GET    | /games/api/events                | 캘린더용 게임 이벤트 (JSON)    | 모두  |
...
| POST   | /crawling/boxscore-range         | 날짜 범위 박스스코어 일괄 수집 | ADMIN |
| POST   | /crawling/cross-validation       | 박스스코어 ↔ 시즌 통계 비교    | ADMIN |
